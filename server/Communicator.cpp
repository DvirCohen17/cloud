...
void Communicator::forgotPassword(std::string msg, SOCKET client_sock)
{
	Action reqDetail = deconstructReq(msg);
	...
	if (!check)
	{
		if (m_database->doesUserExist(reqDetail.userName) && m_database->doesPasswordMatch(reqDetail.userName, reqDetail.oldPass))
		{
			std::string repCode = std::to_string(MC_FORGOT_PASSW_RESP);
			reqDetail.userName = m_database->getUserName(reqDetail.email, -1);
			reqDetail.email = m_database->getEmail(reqDetail.userName);
			m_database->changePassword(reqDetail.userName, reqDetail.oldPass, reqDetail.pass);
			Client* client_handler = new Client(m_database->getUserId(reqDetail.userName), reqDetail.userName, reqDetail.email);
			m_clients[client_sock] = client_handler;
	...
	while (run)
	{
		try
		{
			...
			case MC_FORGOT_PASSW_REQUEST:
				pass = false;
				forgotPassword(newRequest, client_sock);
				break;
