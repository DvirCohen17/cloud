...
void Communicator::createFile(SOCKET client_sock, std::string fileName)
{
	...
	else
	{
		...
		if (CloudConnected)
		{
			writeMessage(5, fileName + ".txt", buffer, bufferSize);
...
void Communicator::deleteFile(SOCKET client_sock, std::string fileName)
{
	...
	else
	{
		...
		if (CloudConnected)
		{
			writeMessage(3, fileName + ".txt", buffer, bufferSize);
			send(m_cloudServerSocket, buffer, bufferSize, 0);
...
void Communicator::getInitialContent(SOCKET client_sock, std::string fileName)
{
	...
	if (CloudConnected)
	{
		...
		writeMessage(2, fileName, buffer, bufferSize);
		send(m_cloudServerSocket, buffer, bufferSize, 0);
		...
		std::string msg = "";
		readMessage(recvbuf, code, msg);
		m_filesData[".\\files\\" + fileName] = msg;
		std::cout << msg << std::endl;
		fileContent = msg;
	}
	else
